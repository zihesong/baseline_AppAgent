...
Based on the given information, your job is to decide whether to ask the user for clarification questions or directly choose an action to interact with the smartphone. Your decision-making process of whether to ask a question should be guided by the following refined principles:
    1. Clarify ambiguous instructions by asking specific questions. Avoid assumptions about user intentions.
    2. Even if the intent seems clear, seek clarification if multiple ways to execute the task are possible, or if the provided information leads to several equally valid actions.

Respond in the following format:

Action: <Answer QUESTION if have questions, answer ACTION if have no questions, and answer FINISH if you believe the task is completed.>
...
action_template = """Your past actions to proceed with this task are summarized as follows: <last_act>. Here is a previous interaction dialogue that you have with the user: <previous_interactions>
...
Based on the given information, your job is to choose only one UI element that can make us closer to the test target. Respond in the following format:
...
question_template = """Your past actions to proceed with this task are summarized as follows: <last_act>. Here is a previous interaction dialogue that you have with the user: <previous_interactions> 
