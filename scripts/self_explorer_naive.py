...
if not task:
    ...
    task_desc = task
    print_with_color(f"Task description: {task_desc}", "blue")

system_prompt = f"You are an agent that is trained to complete certain tasks on a smartphone. The task you need to complete is to {task_desc} on {app}."
...
prompt_template = prompts_naive.self_explore_task_template
...
while round_count < configs["MAX_ROUNDS"]:
    ...
    prompt = re.sub(r"<last_act>", last_act, prompt_template)
    ...
    status, rsp = mllm.get_model_response(system_prompt, prompt, [base64_img_before])
    ...
    if prompt_style == "sequential":
        ...
        if act_name == "QUESTION":
            prompt = prompts_naive.self_explore_task_question_template
            ...
            prompt = prompts_naive.self_explore_task_action_template
            # prompt = re.sub(r"<task_description>", task_desc, prompts_naive.self_explore_task_action_template)
